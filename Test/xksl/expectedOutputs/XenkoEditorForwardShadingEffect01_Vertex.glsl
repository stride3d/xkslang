...
layout(location = 1) in mat3 VS_IN_tangentToWorld;
layout(location = 2) in vec4 VS_IN_Position;
layout(location = 3) in vec4 VS_IN_ScreenPosition;
layout(location = 0) out vec4 VS_OUT_ShadingPosition;
layout(location = 1) out vec3 VS_OUT_normalWS;
layout(location = 2) out mat3 VS_OUT_tangentToWorld;
...
void main()
{
    VS_STREAMS _streams = VS_STREAMS(vec4(0.0), vec3(0.0), vec3(0.0), mat3(vec3(0.0), vec3(0.0), vec3(0.0)), vec4(0.0), vec4(0.0), 0.0, vec4(0.0), vec4(0.0));
